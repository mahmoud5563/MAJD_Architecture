...
router.post('/', auth, authorizeRoles('مدير', 'مدير حسابات'), upload.array('attachments', 5), handleUploadError, async (req, res) => {
    ...
    try {
        ...
        const newTransaction = new Transaction({
            ...
            description: `دفعة مقاول: ${description || 'بدون وصف'} لاتفاقية ${agreement.description || agreement._id}`,
    ...
    try {
        ...
        await Transaction.findOneAndUpdate(
            { contractPayment: payment._id, type: 'دفعة مقاول' },
            {
                ...
                amount: payment.amount,
                description: `دفعة مقاول: ${description || 'بدون وصف'} لاتفاقية ${newAgreement.description || newAgreement._id}`,
                date: payment.date
            }
        );
